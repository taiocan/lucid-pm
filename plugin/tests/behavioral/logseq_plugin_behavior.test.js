...
// child_process is available in this context (Jest/Node.js), so commands run
// via exec() rather than the companion server.
...
test('invariant: extract command is not registered', () => {
  expect(Object.keys(registeredCommands).some((n) => n.toLowerCase().includes('extract'))).toBe(false);
});
